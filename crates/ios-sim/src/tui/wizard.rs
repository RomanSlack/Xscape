...
impl SetupWizard {
    pub async fn run() -> Result<()> {
        ...
        println!(
            "{}",
            "Verifying your ios-sim installation.\n".dimmed()
        );
        ...
        Styles::success("Your ios-sim installation is ready to use!");
        println!();
        println!("   Run: {} {}", "ios-sim".bright_white(), "interactive");
        println!();
    ...
    async fn check_config() -> Result<()> {
        ...
        if config_exists {
            ...
            if create {
                let config = ios_sim_common::CliConfig::default();
    ...
    async fn check_connection() -> Result<()> {
        ...
        match client.health().await {
            ...
            Err(_) => {
                ...
                if config.agent.mode == ios_sim_common::AgentMode::LocalVm {
                    println!();
                    ...
                    Styles::warning("Make sure the xcode-agent is running on your remote Mac");
                }
            }
        }
    ...
    async fn start_vm(config: &ios_sim_common::CliConfig) -> Result<()> {
        ...
        match client.list_simulators().await {
            Ok(response) => {
                ...
                let booted = response
                    .devices
                    .iter()
                    .filter(|d| d.state == ios_sim_common::SimulatorState::Booted)
                    .count();
...
pub async fn quick_status() -> Result<()> {
    ...
    match client.health().await {
        ...
        Err(_) => {
            ...
            Styles::dimmed("Run 'ios-sim vm start' or check your remote Mac");
        }
    }
    println!();
