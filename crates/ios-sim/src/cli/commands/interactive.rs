...
async fn run_project_flow(client: &AgentClient, config: &ios_sim_common::CliConfig) -> Result<()> {
...
/// Build project flow
async fn build_project_flow(client: &AgentClient, config: &ios_sim_common::CliConfig) -> Result<()> {
    // Step 1: Select project
    Screen::clear();
...
/// VM control flow
async fn vm_flow(config: &ios_sim_common::CliConfig) -> Result<()> {
    loop {
        Screen::clear();
...
/// Settings flow
async fn settings_flow(config: &ios_sim_common::CliConfig) -> Result<()> {
    Screen::clear();
