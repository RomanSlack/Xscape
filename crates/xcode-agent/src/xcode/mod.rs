...
use ios_sim_common::{BuildConfiguration, BuildRequest, LogLevel, LogMessage};
use std::path::Path;
...
pub async fn run_build(
    ...
    let _ = log_sender.send(serde_json::to_string(&LogMessage::system_event(
        ios_sim_common::SystemEventType::BuildStarted,
    ...
    if !status.success() {
        let _ = log_sender.send(serde_json::to_string(&LogMessage::system_event(
            ios_sim_common::SystemEventType::BuildFailed,
    ...
    let _ = log_sender.send(serde_json::to_string(&LogMessage::system_event(
        ios_sim_common::SystemEventType::BuildSucceeded,
